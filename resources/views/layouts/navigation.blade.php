<nav x-data="{ open: false }" class="bg-gray-800 border-b border-gray-700">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @can('manage-networks')
                    <x-nav-link :href="route('networks.index')" :active="request()->routeIs('networks.*')" class="text-gray-300 hover:text-white">
                        {{ __('Networks') }}
                    </x-nav-link>
                    @endcan

                    @can('manage-computers')
                    <x-nav-link :href="route('computers.index')" :active="request()->routeIs('computers.*')" class="text-white-100 hover:text-white">
                        {{ __('Computers') }}
                    </x-nav-link>
                    @endcan

                    @can('manage-servers')
                    <x-nav-link :href="route('servers.index')" :active="request()->routeIs('servers.*')" class="text-gray-300 hover:text-white">
                        {{ __('Servers') }}
                    </x-nav-link>
                    @endcan
                </div>
                ...
                <x-dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        <form method="POST" action="{{ route('language.switch') }}">
                            ...
                            <x-dropdown-link :href="route('language.switch')"
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @can('manage-computers')
            <x-responsive-nav-link :href="route('computers.index')" :active="request()->routeIs('computers.*')">
                {{ __('Computers') }}
            </x-responsive-nav-link>
            @endcan
        ...
        <div class="pt-4 pb-1 border-t border-gray-700">
            ...
            <div class="mt-3 space-y-1">
                ...
                <form method="POST" action="{{ route('language.switch') }}">
                    @csrf
                    <input type="hidden" name="language" value="en">
                    <x-responsive-nav-link :href="route('language.switch')"
                            onclick="event.preventDefault();
                                        this.closest('form').submit();">
                        {{ __('English') }}
                    </x-responsive-nav-link>
